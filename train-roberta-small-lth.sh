...
pruning=$1
init_ch=$2
model=lth-exp-small-1shot/roberta-small-baseline-lth-final200-init$init_ch-local$pruning
DATA_DIR=${RAW_DIR}/$name/

mname=${name}/${model}
MODEL_DIR=/tmp-network/user/vnikouli/model/${mname}
init_checkpoint=/tmp-network/user/vnikouli/model/$name/lth-exp-small/roberta-small-baseline/checkpoint$init_ch.pt
final_checkpoint=/tmp-network/user/vnikouli/model/$name/lth-exp-small/roberta-small-baseline/checkpoint_200.pt
#$MODEL_DIR/../../roberta-small-baseline/checkpoint100.pt
mkdir -p ${MODEL_DIR}
TB_DIR=/tmp-network/user/vnikouli/tensorboard_logs/${mname}
echo $MODEL_DIR
mkdir -p ${TB_DIR}
source /home/vnikouli/.bashrc
...
fairseq-train ${DATA_DIR}/bin-tokens --task masked_lm \
    ...
    --lr-scheduler polynomial_decay --lr 0.0005 --warmup-updates 10000 --total-num-update 125000 --max-epoch 100 \
    --dropout 0.3 --attention-dropout 0.1 --weight-decay 0.01 \
    --max-tokens 4096 --update-freq 4 --ddp-backend=no_c10d \
    --max-update 125000 --log-interval 1 --log-format simple  --save-interval 1 --save-interval-updates 10000\
