...
pruning=$1
init_ch=$2
model=lth-exp-small/roberta-small-baseline-lth-init$init_ch-local$pruning
DATA_DIR=${RAW_DIR}/$name/

mname=${name}/${model}
MODEL_DIR=/tmp-network/user/vnikouli/model/${mname}
init_checkpoint=$MODEL_DIR/../roberta-small-baseline/checkpoint1.pt
...
fairseq-train ${DATA_DIR}/bin-tokens --task masked_lm \
    ...
    --max-tokens 4096 --update-freq 8 --ddp-backend=no_c10d \
